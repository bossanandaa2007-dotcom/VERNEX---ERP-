...
check (role in ('Admin', 'Teacher', 'Student', 'Accountant', 'Governing Body', 'Librarian'));
...
drop policy if exists "library_books_manage_admin" on public.library_books;
drop policy if exists "library_books_manage_staff" on public.library_books;
create policy "library_books_manage_staff"
...
create or replace function public.sync_library_book_status()
returns trigger
language plpgsql
...
drop trigger if exists trg_sync_library_book_status on public.library_books;
create trigger trg_sync_library_book_status
before insert or update of total_copies, available_copies on public.library_books
for each row execute function public.sync_library_book_status();
...
begin
  ...
  foreach target_issue_id in array send_library_reminders.issue_ids loop
    ...
    join public.library_books book on book.id = issue.book_id
...
begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    ...
    if not exists (
      ...
      where pubname = 'supabase_realtime'
        and schemaname = 'public'
        and tablename = 'library_reminders'
    ) then
      alter publication supabase_realtime add table public.library_reminders;
    end if;
  end if;
end;
$$;
