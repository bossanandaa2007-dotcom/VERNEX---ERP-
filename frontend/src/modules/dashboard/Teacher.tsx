  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      ...
      <div className="rounded-[1.75rem] border border-emerald-100 bg-emerald-50/80 px-5 py-5 shadow-sm lg:rounded-3xl lg:px-6">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600">Class Teacher Of</p>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-2 lg:grid-cols-5 lg:gap-6">
        {[
          ...
          <div 
            ...
            className={`flex flex-col gap-3 rounded-[1.5rem] border border-slate-100 bg-white p-4 shadow-sm transition-all lg:flex-row lg:items-center lg:gap-4 lg:rounded-2xl lg:p-6 ${
              stat.path ? 'cursor-pointer active:scale-[0.98] lg:hover:shadow-md' : ''
            }`}
          >
          ...
          <div className="space-y-4">
             {isScheduleLoading ? (
               ...
               todaySchedule.map((entry) => (
                  <div key={entry.id} className="flex gap-3 items-center rounded-2xl border border-slate-100 p-3 transition-colors hover:bg-slate-50 lg:gap-4 lg:rounded-xl">
                    <div className="w-20 shrink-0 text-center">
                      <span className="text-sm font-bold text-indigo-600 block">{formatScheduleTime(entry)}</span>
                    </div>
